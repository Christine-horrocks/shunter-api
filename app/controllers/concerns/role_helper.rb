...
module RoleHelper
  ...
  def self.build_timeline(history_hash, current_roles)
    timeline_roles = []
    timeline_roles << { "time-period": I18n.t('.people.roles.held_currently'), "roles": current_roles }

    history_hash[:years].keys.sort.each do |year|
      timeline_roles << { "time-period": "#{I18n.t('.people.previous_roles.held_in')}#{year}#{I18n.t('.people.previous_roles.year_s')}", "roles": RoleHelper.organise_roles(history_hash[:years][year]) }
    end
